...
export async function performNewAnimeCheck() {
    ...
    addAnimesIfNotFound(newAnimes).then(async (res) => {
        ...
        res.failedAnime.forEach(id => failed.push({id, reason: "Fetch error"}));

        for (const id of res.queuedAnime)
            await addAnime(id).catch((err: Error) => failed.push({id, reason: err.message}));

        const failedCount: {[key: string]: number} = {};
        failed.forEach(f => failedCount[f.reason] = (failedCount[f.reason] ?? 0) + 1);
        ...
        if (process.env.DEBUG === "true")
          console.log("Failed dump:", failed);
    });
}
