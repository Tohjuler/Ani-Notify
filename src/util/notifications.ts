...
export default async function sendNotifications(anime: Anime, episode: EpisodeInfo) {
    ...
    for (const user of users) {
        ...
        if (user.ntfy_url)
            sendNtfyNotification(anime, episode, user.ntfy_url);
    }
}

function sendDiscordNotification(anime: Anime, ep: EpisodeInfo, webhook: string) {
    const message = `Title: ${ep.title ?? "N/A"}
    ${ep.description ? `Description: ${ep.description}` : ""}
    Language: ${ep.dub ? "Dub" : "Sub"}
    You can watch it on ${ep.providers.split(",").map((p) => p[0].toUpperCase() + p.slice(1)).join(", ")}
    `
        ...
        embeds: [
            {
                ...
                author: {
                    name: "Ani-Notify",
                    icon_url: "http://cloud.tohjuler.dk/s/tEKyqLNxmX7Adrr/download/Zetsu.jpg"
                },
