...
export async function fetchAiringAnimes(page = 1): Promise<string[]> {
	const from = Math.floor(new Date().getTime() / 1000);
	// 3 days in seconds
	const to =
		from +
		(Number.parseInt(process.env.AUTO_REGISTER_CHECK_DAYS as string) || 2) *
			24 *
			60 *
			60;
...
`;
		...
		let animes: string[] = response.data.data.Page.airingSchedules
			.filter(
				(a: { media: { status: string } }) => a.media.status === "RELEASING",
			)
			.map((a: { media: { id: number } }) => `${a.media.id}`); // Cheasy way to get it as string
